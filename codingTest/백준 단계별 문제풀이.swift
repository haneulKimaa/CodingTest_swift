...
// 카카오 2022 공채 - 테스트 1번
// [[1, 4], [3, 4], [3, 10]]
func kakao2022TestNum1() {
    var readline222 = readLine()!.replacingOccurrences(of: "], [", with: "-").replacingOccurrences(of: "[[", with: "").replacingOccurrences(of: "]]", with: "").split(separator: "-").map {$0.replacingOccurrences(of: ", ", with: "+").split(separator: "+").map { Int($0)! } }
    var xValue: Int = 0
    var yValue: Int = 0
    var resultArray: [[Int]] = []
    for i in 0..<readline222.count-1 {
        for j in i+1..<readline222.count {
            if readline222[i][0] == readline222[j][0] {
                xValue = readline222[i][0]
                break
            }
            if readline222[i][1] == readline222[j][1] {
                yValue = readline222[i][1]
                break
            }
        }
    }
    resultArray.append([xValue, yValue])
...
//let terms = readLine().map { $0.split(separator: " ").map { String($0) } }
//let privacies = readLine().map { $0.split(separator: " ").map { String($0) } }

func kakaoNum1() {
    let today = readLine()!
    var terms = readLine()!.replacingOccurrences(of: "[", with: "").replacingOccurrences(of: "]", with: "").replacingOccurrences(of: "\"", with: "").replacingOccurrences(of: ", ", with: "-").split(separator: "-").map { String($0) }
    let privacies = readLine()!.replacingOccurrences(of: "[", with: "").replacingOccurrences(of: "]", with: "").replacingOccurrences(of: "\"", with: "").replacingOccurrences(of: ", ", with: "-").split(separator: "-").map { String($0) }
    
    func solution(_ today:String, _ terms:[String], _ privacies:[String]) -> [Int] {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "YYYY.MM.dd"
        let todayDate: Date = dateFormatter.date(from: today)!
        let termArray = terms.map { $0.replacingOccurrences(of: " ", with: "+").split(separator: "+") }
        let privaciesArray = privacies.map { $0.replacingOccurrences(of: " ", with: "+").split(separator: "+") }
        var result: [Int] = []
        for i in 0..<privaciesArray.count {
            for j in 0..<termArray.count {
                if privaciesArray[i][1] == termArray[j][0] {
                    let privacyDate = dateFormatter.date(from: String(privaciesArray[i][0]))
                    
                    let addedDate = Calendar.current.date(byAdding: .month, value: Int(termArray[j][1])!, to: privacyDate!)!
                    
                    if todayDate.compare(addedDate) == .orderedDescending || todayDate.compare(addedDate) == .orderedSame {
                        result.append(i+1)
                    }
                }
            }
...
func kakaoNum2() {
    ...
    let n = Int(readLine()!)
    var pickups = readLine()!.replacingOccurrences(of: "[", with: "").replacingOccurrences(of: "]", with: "").replacingOccurrences(of: ", ", with: "+").split(separator: "+").map { Int($0)! }
    var deliveries = readLine()!.replacingOccurrences(of: "[", with: "").replacingOccurrences(of: "]", with: "").replacingOccurrences(of: ", ", with: "+").split(separator: "+").map { Int($0)! }
    var result: Int64 = 0
    
    func solution(_ cap:Int, _ n:Int, _ deliveries:[Int], _ pickups:[Int]) -> Int64 {
        var deliveriesArr = deliveries
        var pickupsArr = pickups
        var capValue = cap
        var result: Int64 = 0
        
        while deliveriesArr != Array(repeating: 0, count: deliveries.count) &&
                pickupsArr != Array(repeating: 0, count: pickups.count) {
            var maxLength = 0
            for i in (0...deliveriesArr.count-1).reversed() {
                
                if deliveriesArr[i] <= capValue  {
                    if deliveriesArr[i] != 0 {
                        capValue -= deliveriesArr[i]
                        deliveriesArr[i] = 0
                        if maxLength < i {
                            maxLength = i
                            result += Int64(((i+1) * 2))
                    ...
                    capValue = 0
                    // 택배가 부족할 때 물류창고로 돌아가면서 수거할 수 있는 것 수거
                    for j in (0...i+1).reversed() {
                        // 공간이 남으면 상자 수거
                        if pickupsArr[j] <= (cap-capValue) {
                            capValue += pickupsArr[j]
                            pickupsArr[j] = 0
                ...
                if deliveriesArr[i] == 0 && pickupsArr[i] != 0 && (cap-capValue) >= pickupsArr[i] {
                    capValue += pickupsArr[i]
                    pickupsArr[i] = 0
                    if maxLength < i {
                        maxLength = i
                        result += Int64(((i+1) * 2))
...
var users = readLine()!.replacingOccurrences(of: "], [", with: "-").replacingOccurrences(of: "[[", with: "").replacingOccurrences(of: "]]", with: "").split(separator: "-").map {$0.replacingOccurrences(of: ", ", with: "+").split(separator: "+").map { Int($0)! } }

var emoticons = readLine()!.replacingOccurrences(of: ", ", with: "+").split(separator: "+").map { $0.replacingOccurrences(of: "[", with: "").replacingOccurrences(of: "]", with: "").replacingOccurrences(of: "", with: "") }.map { Int($0)! }
var resultArr: [Int] = []
...
func solution(_ users:[[Int]], _ emoticons:[Int]) -> [Int] {
    ...
    for i in 0..<users.count {
        print(users[i][0]/100)
        let doubleUser = Double(users[i][0])
        print(Double(price) * (doubleUser/100))
        if (Double(price) * (doubleUser/100)) > Double(users[i][1]) {
            person += 1
        }
        else {
            resultPrice += price
    ...
    resultArr.append(person)
    resultArr.append(resultPrice)
    return resultArr
}
